...
class StudentViewSet(viewsets.ModelViewSet):
    ...
    def report_card(self, request, pk=None):
        ...
        def get_grade_info(score):
            ...
            if thresholds:
                ...
                if rounded_score < sorted_t[0].min_score:
                    return {"grade": sorted_t[0].grade, "points": sorted_t[0].points, "remarks": sorted_t[0].remarks}
                if rounded_score > sorted_t[-1].max_score:
                    return {"grade": sorted_t[-1].grade, "points": sorted_t[-1].points, "remarks": sorted_t[-1].remarks}
        ...
        attendance_summary = {
            ...
            "percentage": round((present_count / total_days * 100), 1) if total_days > 0 else 100
        }
        ...
        remarks = {
            "teacher": report_meta.teacher_comment if report_meta else "No teacher comment provided.",
            "principal": report_meta.principal_comment if report_meta else "No principal comment provided.",
        ...
        return Response({
            "student": {
                ...
                "class_teacher": student.stream.teacher.full_name if (student.stream and student.stream.teacher) else "Not Assigned",
                "email": student.user.email,
                "photo": student.photo.url if student.photo else None,
                "curriculum": student.curriculum,
                "guardians": [{"first_name": g.first_name, "last_name": g.last_name, "phone_number": g.phone_number} for g in student.guardians.all()],
            },
            "exam": {
                "id": exam.id,
                "name": exam.name,
