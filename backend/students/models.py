...
class Student(SoftDeleteModel):
    ...
    upi_number = models.CharField(
        max_length=50, null=True, blank=True, verbose_name="Nemis UPI"
    )
...
class Guardian(models.Model):
    ...
    phone_regex = RegexValidator(
        regex=r"^\+?1?\d{9,15}$", message="Phone number format: '+999999999'."
    )
...
class MedicalRecord(models.Model):
    ...
    student = models.OneToOneField(
        Student, on_delete=models.CASCADE, related_name="medical_record"
    )
...
class StudentTransfer(models.Model):
    ...
    transfer_letter = models.FileField(
        upload_to="transfer_letters/", null=True, blank=True
    )
