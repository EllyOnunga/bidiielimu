...
class APIKeySerializer(serializers.ModelSerializer):
    class Meta:
        model = APIKey
        fields = ['id', 'name', 'key', 'key_type', 'is_active', 'expires_at', 'last_used_at', 'created_at']
...
class SchoolBasicSerializer(serializers.ModelSerializer):
    ...
    def get_domain(self, obj):
        ...
        return domain.domain if domain else f"{obj.schema_name.replace('_', '-')}.localhost"
...
class UserSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = [
            'id', 'email', 'first_name', 'last_name', 'role', 'role_details',
            'school', 'school_details', 'phone_number', 'is_email_verified',
            'profile_picture_url', 'github_username', 'bio', 'job_title'
        ]
...
class RegisterSerializer(serializers.ModelSerializer):
    ...
    curriculum = serializers.ChoiceField(choices=School.CURRICULUM_CHOICES, write_only=True, default='CBC')
    school_details = serializers.SerializerMethodField()

    class Meta:
        model = User
        fields = ['email', 'password', 'first_name', 'last_name', 'school_name', 'curriculum', 'school_details']
    ...
    def create(self, validated_data):
        ...
        try:
            school = School.objects.create(name=school_name, schema_name=schema_name, curriculum=curriculum)
        except IntegrityError:
            raise serializers.ValidationError({"school_name": ["A school with this name already exists. Please choose a unique name."]})
        ...
        try:
            ...
            school.delete()
            raise serializers.ValidationError({"school_name": ["A domain for this school name already exists. Please choose a unique name."]})
        ...
        if request:
            ...
            if tenant:
                if tenant.schema_name == 'public':
                    ...
                    if not (self.user.is_superuser or self.user.role_name == 'SUPER_ADMIN'):
                        raise serializers.ValidationError({
