...
from .models import User, SMSDevice
...
class SMSOTPVerifySetupView(views.APIView):
    ...
    def post(self, request):
        ...
        if OTPService.verify_otp(request.user, otp):
            device = SMSDevice.objects.filter(user=request.user, confirmed=False).first()
            if device:
                device.confirmed = True
                device.save()
                return Response({"message": "SMS 2FA enabled successfully"})
            return Response({"message": "OTP verified successfully"}) # Case for general verification
