...
class AttendanceAnalyticsService:
    ...
    def get_chronic_absentees(school_id, threshold=0.8):
        ...
        # Simplified logic
        absentee_data = (
            PeriodAttendance.objects.filter(student__school_id=school_id)
            .values("student")
            .annotate(
        ...
        for item in absentee_data:
            ...
            if rate < threshold:
                chronic.append(
                    {"student_id": item["student"], "rate": round(rate * 100, 1)}
                )
        return chronic
    ...
    def correlate_with_performance(student_id):
        ...
        mean_score = ExamRanking.objects.filter(student_id=student_id).aggregate(
            avg=Avg("mean_score")
        )["avg"]

        return {"attendance_rate": attendance_rate, "academic_performance": mean_score}
