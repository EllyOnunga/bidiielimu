...
class TimetableGenerator:
    ...
    def generate_for_stream(
        stream_id, start_time="08:00", end_time="16:00", period_duration=40
    ):
        ...
        for assignment in assignments:
            req = CurriculumRequirement.objects.filter(
                grade_level=stream.grade_level, subject=assignment.subject
            ).first()
        ...
        while curr + timedelta(minutes=period_duration) <= end_dt:
            periods.append(
                (curr.time(), (curr + timedelta(minutes=period_duration)).time())
            )
        ...
        for assignment in slots_to_fill:
            ...
            for day in days:
                for p_start, p_end in periods:
                    ...
                    if ScheduleSlot.objects.filter(
                        stream=stream, day_of_week=day, start_time=p_start
                    ).exists():
                        continue

                    # Check Teacher availability
                    if not TimetableGenerator.is_teacher_available(
                        assignment.teacher, day, p_start, p_end
                    ):
                        continue
            ...
            if not placed:
                failed_assignments.append(
                    f"{assignment.subject.name} ({assignment.teacher})"
                )
    ...
    def is_teacher_available(teacher, day, start, end):
        ...
        is_booked = ScheduleSlot.objects.filter(
            teacher=teacher, day_of_week=day, start_time__lt=end, end_time__gt=start
        ).exists()
    ...
    def get_available_room(day, start, end):
        ...
        for room in rooms:
            is_occupied = ScheduleSlot.objects.filter(
                classroom=room, day_of_week=day, start_time__lt=end, end_time__gt=start
            ).exists()
            if not is_occupied:
                return room
        return None
