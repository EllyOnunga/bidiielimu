...
class Strand(models.Model):
    subject = models.ForeignKey(
        Subject, on_delete=models.CASCADE, related_name="strands"
    )
...
class SubStrand(models.Model):
    strand = models.ForeignKey(
        Strand, on_delete=models.CASCADE, related_name="sub_strands"
    )
    name = models.CharField(max_length=200)
...
class SubjectAssignment(models.Model):
    teacher = models.ForeignKey(
        "teachers.Teacher", on_delete=models.CASCADE, related_name="subject_assignments"
    )
    subject = models.ForeignKey(
...
class ScheduleSlot(models.Model):
    ...
    stream = models.ForeignKey(
        Stream, on_delete=models.CASCADE, related_name="schedule"
    )
    ...
    def clean(self):
        ...
        if teacher_overlap.exists():
            raise ValidationError(
                f"Teacher {self.teacher} is already booked at this time."
            )
        ...
        if self.classroom:
            ...
            if room_overlap.exists():
                raise ValidationError(
                    f"Classroom {self.classroom} is already occupied at this time."
                )
        ...
        if stream_overlap.exists():
            raise ValidationError(
                f"Stream {self.stream} already has a lesson at this time."
            )
...
class CurriculumRequirement(models.Model):
    grade_level = models.ForeignKey(
        GradeLevel, on_delete=models.CASCADE, related_name="requirements"
    )
    subject = models.ForeignKey(Subject, on_delete=models.CASCADE)
...
class Substitution(models.Model):
    schedule_slot = models.ForeignKey(
        ScheduleSlot, on_delete=models.CASCADE, related_name="substitutions"
    )
    original_teacher = models.ForeignKey(
        "teachers.Teacher", on_delete=models.CASCADE, related_name="substitutions_given"
    )
    substitute_teacher = models.ForeignKey(
        "teachers.Teacher", on_delete=models.CASCADE, related_name="substitutions_taken"
    )
