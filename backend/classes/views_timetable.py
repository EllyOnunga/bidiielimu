...
class TimetableAutoGenerateView(views.APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not stream_id:
            return Response(
                {"detail": "stream_id is required."}, status=status.HTTP_400_BAD_REQUEST
            )
...
class TimetableConflictView(views.APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        return Response(
            {"status": "conflict", "errors": serializer.errors},
            status=status.HTTP_200_OK,
        )  # We return 200 so the frontend can handle the data
