...
class ReceiptService:
    ...
    def generate_receipt_text(payment_id):
        ...
        school_name = connection.tenant.name.upper() if hasattr(connection, 'tenant') and connection.tenant else "ELIMUHUB ACADEMY"
    ...
    def generate_receipt_pdf(payment_id):
        ...
        primary_color = settings_obj.accent_color if settings_obj and settings_obj.accent_color else "#2DD4BF"
        ...
        c.drawString(1 * inch, 8.4 * inch, f"Student: {payment.student.first_name} {payment.student.last_name}")
        c.drawString(1 * inch, 8.0 * inch, f"Adm No: {payment.student.admission_number}")
        
        c.drawString(4.5 * inch, 9.2 * inch, f"Amount Paid: {payment.currency} {payment.amount}")
        c.drawString(4.5 * inch, 8.8 * inch, f"Method: {payment.get_payment_method_display()}")
        
        if payment.invoice:
            c.drawString(4.5 * inch, 8.4 * inch, f"Balance: {payment.currency} {payment.invoice.balance}")
