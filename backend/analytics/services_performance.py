...
class PerformanceAnalyticsService:
    ...
    def get_subject_performance_distribution(subject_id, exam_id):
        ...
        marks = Mark.objects.filter(subject_id=subject_id, exam_id=exam_id).values_list(
            "score", flat=True
        )
        return list(marks)
    ...
    def get_cohort_trends(grade_level_id):
        ...
        rankings = (
            ExamRanking.objects.filter(student__stream__grade_level_id=grade_level_id)
            .values("exam__name", "exam__date")
            .annotate(avg_score=Avg("mean_score"))
            .order_by("exam__date")
        )
    ...
    def get_teacher_effectiveness(teacher_id):
        ...
        teacher_avg = (
            Mark.objects.filter(subject__teacher_id=teacher_id).aggregate(
                avg=Avg("score")
            )["avg"]
            or 0
        )
    ...
    def get_school_kpis():
        ...
        passing = (
            ExamRanking.objects.filter(mean_score__gte=50)
            .values("student")
            .distinct()
            .count()
        )
        ...
        return {
            ...
            "mean_exam_score": round(
                ExamRanking.objects.aggregate(avg=Avg("mean_score"))["avg"] or 0, 2
            ),
            "pass_rate": pass_rate,
        }
