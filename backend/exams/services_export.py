...
from .models import Exam, ExamRanking, Mark
...
class MarkSheetExportService:
    ...
    def generate_mark_sheet_csv(exam_id):
        ...
        header.extend(
            ["Total Marks", "Mean Score", "Mean Grade", "Stream Rank", "Class Rank"]
        )
        writer.writerow(header)

        # Data Rows
        rankings = ExamRanking.objects.filter(exam=exam).select_related(
            "student", "student__stream"
        )
        for rank in rankings:
            ...
            for sub in subjects:
                mark = Mark.objects.filter(
                    exam=exam, student=student, subject=sub
                ).first()
                if mark:
