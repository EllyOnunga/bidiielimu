...
from .models import Exam, ExamRanking, GradeThreshold, GradingSystem, Mark
...
class MarkSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if user.role_name == "TEACHER":
            ...
            is_assigned = ScheduleSlot.objects.filter(
                teacher__user=user, stream=student.stream, subject=subject
            ).exists()

            is_class_teacher = (
                student.stream.teacher.user == user
    ...
    def _get_threshold(self, obj):
        ...
        threshold = obj.exam.grading_system.thresholds.filter(
            min_score__lte=rounded_score, max_score__gte=rounded_score
        ).first()
