...
def audit_tenant(schema_name):
    ...
    with schema_context(schema_name):
        ...
        if teachers_no_user.exists():
            issues.append(f"CRITICAL: {teachers_no_user.count()} teachers have no linked User account.")
        ...
        if students_no_stream.exists():
            issues.append(f"WARNING: {students_no_stream.count()} students are not assigned to any stream.")
            
        # 3. Streams Check
        streams = Stream.objects.all()
        streams_no_teacher = streams.filter(teacher__isnull=True)
        if streams_no_teacher.exists():
            issues.append(f"INFO: {streams_no_teacher.count()} streams have no class teacher assigned.")
        ...
        if assignments.count() == 0:
            issues.append("CRITICAL: No Subject Assignments found. Teachers will not be able to record marks.")
            
        # 5. Exams Check
        exams = Exam.objects.all()
        exams_no_grading = exams.filter(grading_system__isnull=True)
        if exams_no_grading.exists():
            issues.append(f"WARNING: {exams_no_grading.count()} exams have no grading system linked.")
