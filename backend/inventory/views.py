...
class InventoryItemViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.role_name not in INVENTORY_ALLOWED_ROLES:
            ...
            raise PermissionDenied(
                "You do not have permission to access inventory records."
            )
    ...
    def library_stats(self, request):
        ...
        items = list(
            library_qs.values(
                "id", "name", "quantity", "unit", "min_threshold", "last_restock"
            )
        )
    ...
    def books_count(self, request):
        ...
        """
        total = (
            InventoryItem.objects.filter(category="LIBRARY").aggregate(
                total=Sum("quantity")
            )["total"]
            or 0
        )
...
class BookIssueViewSet(viewsets.ModelViewSet):
    ...
    def overdue(self, request):
        ...
        overdue_qs = BookIssue.objects.filter(status="OVERDUE").select_related(
            "item", "student"
        )
...
class ProcurementLogViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.role_name not in INVENTORY_ALLOWED_ROLES:
            ...
            raise PermissionDenied(
                "You do not have permission to access procurement logs."
            )
