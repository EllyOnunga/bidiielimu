...
class AIReportService:
    ...
    def generate_narrative_draft(report_id):
        ...
        # 1. Gather Context
        marks = Mark.objects.filter(student=student, exam=report.exam).select_related(
            "subject"
        )
        performance_summary = ", ".join(
            [f"{m.subject.name}: {m.score}%" for m in marks]
        )
        ...
        strength_text = (
            f" particularly excelling in {', '.join(strength_subjects)}"
            if strength_subjects
            else ""
        )
        improvement_text = (
            f" However, {student.first_name} should focus more on {', '.join(improvement_subjects)} to improve the overall grade."
            if improvement_subjects
            else " Maintain this level of dedication across all units."
        )
