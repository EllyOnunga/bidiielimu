...
from exams.models import Mark, ExamRanking, GradeThreshold
...
class ReportCardService:
    ...
    def get_report_card_data(student_id, exam_id):
        student = Student.objects.select_related('current_class', 'stream').get(id=student_id)
        
        # 1. Get Marks with Grades
        marks_qs = Mark.objects.filter(student_id=student_id, exam_id=exam_id).select_related('subject')
        ...
        if exam and exam.grading_system:
            thresholds = list(GradeThreshold.objects.filter(grading_system=exam.grading_system).order_by('-min_score'))
        ...
        for mark in marks_qs:
            ...
            subjects_data.append({
                ...
                'grade': grade,
                'remarks': remarks,
                'teacher': mark.subject.teacher.full_name if hasattr(mark.subject, 'teacher') and mark.subject.teacher else "N/A"
            })

        # 2. Get Ranking Info
        ranking = ExamRanking.objects.filter(student_id=student_id, exam_id=exam_id).first()
        ranking_data = {
        ...
        # 4. Get Remarks
        report = StudentReport.objects.filter(student_id=student_id, exam_id=exam_id).first()
        remarks_data = {
            'teacher_comment': report.teacher_comment if report else "No teacher comment provided.",
            'principal_comment': report.principal_comment if report else "No principal comment provided.",
