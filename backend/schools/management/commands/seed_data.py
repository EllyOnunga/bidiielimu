...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write(f"Seeding data for schema: {School.objects.first().schema_name if School.objects.exists() else 'unknown'}")
        ...
        for i in range(5):
            ...
            if not User.objects.filter(email=email).exists():
                ...
                Teacher.objects.create(
                    ...
                    joining_date=timezone.now() - timedelta(days=random.randint(100, 500))
                )
        ...
        for i in range(20):
            ...
            if not Student.objects.filter(admission_number=admission_number).exists():
                Student.objects.create(
                    ...
                    date_of_birth=timezone.now() - timedelta(days=random.randint(2000, 4000)),
        ...
        if grade_levels:
            ...
            for i in range(30):
                ...
                FeePayment.objects.create(
                    ...
                    payment_date=timezone.now().date() - timedelta(days=random.randint(0, 120))
                )
