...
from config.caching import cache_tenant_page, cache_response
...
class SchoolViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        domain_name = self.request.data.get('domain_url', f"{tenant.schema_name}.elimuhub.com")
    ...
    def dashboard_stats(self, request):
        ...
        s_last = Student.objects.filter(created_at__gte=last_month_start, created_at__lt=this_month_start).count()
        
        # Teacher Trend
        t_this = Teacher.objects.filter(joining_date__gte=this_month_start).count()
        t_last = Teacher.objects.filter(joining_date__gte=last_month_start, joining_date__lt=this_month_start).count()
        
        # Fees Trend
        f_this = FeePayment.objects.filter(payment_date__gte=this_month_start).aggregate(total=Sum('amount'))['total'] or 0
        f_last = FeePayment.objects.filter(payment_date__gte=last_month_start, payment_date__lt=this_month_start).aggregate(total=Sum('amount'))['total'] or 0
        ...
        library_low_stock = library_qs.filter(quantity__lte=DbF('min_threshold')).count()
    ...
    def _get_global_analytics(self):
        ...
        for school in schools:
            try:
                with tenant_context(school):
                    ...
                    sub_perf = Mark.objects.values('subject__name').annotate(avg=Avg('score'))
                    ...
                    # 2. Attendance
                    att = DailyAttendance.objects.filter(date__gte=thirty_days_ago).values('date').annotate(
                    ...
                    dist = Student.objects.values('stream__grade_level__name').annotate(c=Count('id'))
                    for item in dist:
                        name = item['stream__grade_level__name']
                        if not name: continue
                        global_class_dist[name] = global_class_dist.get(name, 0) + item['c']
            except Exception:
                continue
        ...
        attendance_trend = [
            {"date": d.strftime('%d %b'), "present": v['present'], "absent": v['absent']}
            for d, v in sorted(global_attendance.items())
        ]
        
        class_distribution = [
            {"name": name, "value": count}
            for name, count in global_class_dist.items()
        ]
    ...
    def super_admin_stats(self, request):
        from django.db import connection
        print(f"DEBUG: super_admin_stats called by {request.user.email} (Role: {request.user.role_name}) on schema: {connection.schema_name}")
        if request.user.role_name != 'SUPER_ADMIN':
             return Response({"detail": "Permission denied."}, status=403)
