      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-4">
          {feeItems.map((item) => (
            <div
              key={item.id}
              className="glass p-6 rounded-[32px] border border-white/5 flex items-center justify-between group hover:border-white/10 transition-all"
            >
        ...
        <div className="space-y-8">
          <div className="glass p-10 rounded-[48px] border border-white/10 bg-primary-500/5 space-y-10">
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center px-4 py-3 bg-white/5 rounded-2xl">
                <span className="text-xs font-bold text-primary-200/60">
                  Compulsory
                </span>
                <span className="text-xs font-black text-white">
                  KES {(totalFees - 12000).toLocaleString()}
                </span>
              </div>
              <div className="flex justify-between items-center px-4 py-3 bg-white/5 rounded-2xl">
                <span className="text-xs font-bold text-primary-200/60">
          ...
          <div className="glass p-8 rounded-[40px] border border-white/5 space-y-6">
            ...
            <p className="text-primary-200/40 text-sm leading-relaxed">
              This structure will be applied to{" "}
              <span className="text-white font-bold">1,240 students</span> in
              Form 1-4. The total expected revenue for Term 2 is{" "}
              <span className="text-emerald-400 font-black">KES 64.5M</span>.
            </p>
          </div>
        </div>
      </div>
