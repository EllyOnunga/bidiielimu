...
export const QuizBuilder = ({ isOpen, onClose, onSuccess, initialData }: QuizBuilderProps) => {
  ...
  const [formData, setFormData] = useState({
    ...
    questions: (initialData?.questions || [
      { text: '', question_type: 'MCQ', points: 1, options: ['', ''], correct_answer: '0' }
    ]) as QuestionDraft[]
  });
  ...
  const removeOption = (qIndex: number, oIndex: number) => {
    ...
    if (newQuestions[qIndex].correct_answer === String(oIndex)) {
        ...
        newQuestions[qIndex].correct_answer = String(parseInt(newQuestions[qIndex].correct_answer) - 1);
    }
    setFormData({ ...formData, questions: newQuestions });
  };
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={initialData ? "Reconfigure Assessment" : "Intelligence Module Synthesis"}
    ...
    >
      <form onSubmit={handleSubmit} className="space-y-10 mt-8 max-h-[70vh] overflow-y-auto pr-4 custom-scrollbar">
        {/* Meta Section */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 p-6 rounded-[32px] bg-white/5 border border-white/5">
          <div className="space-y-4">
            <div className="space-y-2">
              <label className="text-[10px] font-black text-primary-200/30 uppercase tracking-widest ml-1">Assessment Title</label>
              <input 
                required
                type="text"
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-black text-primary-200/30 uppercase tracking-widest ml-1">Contextual Description</label>
              <textarea 
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              <label className="text-[10px] font-black text-primary-200/30 uppercase tracking-widest ml-1">Target Subject Domain</label>
              <select 
                required
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-black text-primary-200/30 uppercase tracking-widest ml-1">Mission Duration (Minutes)</label>
              <input 
                required
                type="number"
        ...
        <div className="space-y-8">
          ...
          <div className="space-y-6">
            {formData.questions.map((q, qIndex) => (
              <div key={qIndex} className="p-8 rounded-[40px] bg-white/5 border border-white/5 relative group hover:border-white/10 transition-all">
                <button 
                  type="button" 
                ...
                <div className="space-y-6">
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1 space-y-2">
                      <label className="text-[10px] font-black text-primary-200/30 uppercase tracking-widest">Question Payload</label>
                      <input 
                        required
                        type="text"
                        value={q.text}
                    ...
                    <div className="w-32 space-y-2">
                      <label className="text-[10px] font-black text-primary-200/30 uppercase tracking-widest">Weight (Pts)</label>
                      <input 
                        required
                        type="number"
                        value={q.points}
                  ...
                  <div className="space-y-4 ml-12">
                    <label className="text-[10px] font-black text-primary-200/30 uppercase tracking-widest flex items-center justify-between">
                      Option Set
                      <span className="text-[9px] text-amber-500/50">Mark the correct answer using the radio selection</span>
                    </label>
                    <div className="grid grid-cols-1 gap-3">
                      {q.options.map((opt, oIndex) => (
                        <div key={oIndex} className="flex items-center gap-4 group/opt">
                          <button 
                            type="button"
                            onClick={() => updateQuestion(qIndex, { correct_answer: String(oIndex) })}
                          ...
                          >
                            {q.correct_answer === String(oIndex) && <CheckCircle2 className="w-4 h-4 text-emerald-500" />}
                          </button>
                          <input 
                            required
                            type="text"
        ...
        <div className="flex gap-4 pt-10 sticky bottom-0 bg-slate-950 pb-4">
           <Button type="button" variant="ghost" onClick={onClose} className="flex-1 rounded-2xl h-14 border border-white/5">
             Discard Draft
           </Button>
           <Button type="submit" className="flex-[2] rounded-2xl h-14 shadow-premium bg-amber-500 hover:bg-amber-600 font-black uppercase tracking-widest flex items-center justify-center gap-3" disabled={isSubmitting}>
             {isSubmitting ? <Plus className="w-6 h-6 animate-spin" /> : <Save className="w-6 h-6" />}
