  ...
  const handleDragEnd = async (event: any) => {
    ...
    try {
      ...
      if (checkRes.data.status === "conflict") {
        toast.error(
          "Conflict detected: " + JSON.stringify(checkRes.data.errors),
        );
        return;
      }
      ...
      setSlots((prev) =>
        prev.map((s) =>
          s.id === slotId
            ? { ...s, day_of_week: days.indexOf(newDay), start_time: newTime }
            : s,
        ),
      );
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-primary-200/50 text-sm">
            Drag and drop to reschedule or use AI to automate
          </p>
        </div>
      ...
      >
        <div className="grid grid-cols-[100px_repeat(5,1fr)] bg-white/5 border border-white/10 rounded-[24px] overflow-hidden">
          ...
          {days.map((day) => (
            <div
              key={day}
              className="p-4 border-r border-b border-white/10 bg-white/5 text-center text-xs font-black text-primary-400 uppercase tracking-widest"
            >
              {day}
            </div>
          ))}
            ...
            <React.Fragment key={time}>
              ...
              {days.map((day) => {
                ...
                const slot = slots.find(
                  (s) =>
                    s.day_of_week === dayIdx && s.start_time.startsWith(time),
                );
                return (
          ...
          {activeId ? (
            <div className="p-3 bg-primary-500 text-white rounded-xl shadow-2xl opacity-90 scale-105 border border-white/20">
              <h4 className="text-xs font-black truncate">
                {slots.find((s) => s.id === activeId)?.subject_name}
              </h4>
            </div>
          ) : null}
        </DragOverlay>
