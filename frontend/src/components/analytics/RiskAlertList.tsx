    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3 mb-8">
        ...
        <div>
          ...
          <p className="text-primary-200/40 text-sm font-medium">
            Students flagged for immediate intervention
          </p>
        </div>
      </div>
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
              <div className="flex items-center gap-5">
                ...
                <div>
                  <div className="flex items-center gap-3">
                    ...
                    <span
                      className={`px-2 py-0.5 rounded-lg text-[10px] font-black uppercase tracking-tighter border ${getRiskStyles(risk.risk_level)}`}
                    >
                      {risk.risk_level}
                    </span>
                  </div>
                  <p className="text-sm text-primary-200/40 font-bold uppercase tracking-widest mt-1">
                    {risk.grade}
                  </p>
                </div>
              </div>
              ...
              <div className="flex flex-wrap gap-2 max-w-md">
                {risk.reason_summary.map((reason, i) => (
                  <span
                    key={i}
                    className="px-3 py-1 bg-white/5 rounded-full text-[10px] font-bold text-white/40 border border-white/5"
                  >
                    {reason}
                  </span>
                ))}
              ...
              <div className="flex items-center gap-3">
                <div className="text-right mr-4">
                  <p className="text-[10px] font-black text-primary-200/30 uppercase tracking-widest">
                    AI Confidence
                  </p>
            ...
            <div className="space-y-8">
              <div className="flex items-center gap-6">
                ...
                <div>
                  ...
                  <p className="text-primary-200/40 font-medium">
                    For {selectedRisk.student_name}
                  </p>
                </div>
              </div>

              <div className="space-y-4">
                <label className="text-xs font-black text-primary-200/30 uppercase tracking-widest ml-4">
                  Intervention Type
                </label>
                <div className="grid grid-cols-2 gap-4">
                  {[
                    ...
                    <button
                      key={type}
                      className="p-4 bg-white/5 border border-white/5 rounded-2xl text-white font-bold hover:bg-primary-500/20 hover:border-primary-500/50 transition-all text-left"
                    >
                      {type}
                    </button>
                  ))}
