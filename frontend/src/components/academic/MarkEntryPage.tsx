...
export const MarkEntryPage = () => {
  ...
  const { data: { exams = [], assignments = [] } = {} } = useQuery({
    ...
    queryFn: async () => {
      ...
      return {
        exams: Array.isArray(examsRes.data) ? examsRes.data : (examsRes.data.results || []),
  ...
  const saveMutation = useMutation({
    ...
    onMutate: async (newMarks) => {
      ...
      await queryClient.cancelQueries({ queryKey: ['marks', selectedExam, selectedSubject, selectedStream] });
      // Snapshot previous value
      const previousMarks = queryClient.getQueryData(['marks', selectedExam, selectedSubject, selectedStream]);
      // Optimistically update
      queryClient.setQueryData(['marks', selectedExam, selectedSubject, selectedStream], newMarks);
    ...
    onError: (_err, _newMarks, context) => {
      ...
      if (context?.previousMarks) {
        queryClient.setQueryData(['marks', selectedExam, selectedSubject, selectedStream], context.previousMarks);
    ...
    onSettled: () => {
      queryClient.invalidateQueries({ queryKey: ['marks', selectedExam, selectedSubject, selectedStream] });
    }
  });

  const handleScoreChange = (id: string, newScore: string) => {
    setMarks(prev => prev.map((m: StudentMark) => m.id === id ? { ...m, score: newScore } : m));
  };
            ...
            {loading ? (
              <tr>
                <td colSpan={5} className="py-20 text-center">
                  ...
                  <p className="text-primary-200/40 font-bold uppercase tracking-widest text-xs">Fetching students...</p>
                </td>
              </tr>
            ) : filteredMarks.length === 0 ? (
              <tr>
                <td colSpan={5} className="py-20 text-center text-primary-200/20 italic font-medium">
                  {(!selectedExam || !selectedSubject) ? 'Please select an exam and subject to begin.' : 'No students found in this stream.'}
                </td>
              </tr>
