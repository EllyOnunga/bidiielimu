...
export const IncidentForm = ({ open, onClose }: IncidentFormProps) => {
  ...
  const students: Student[] = Array.isArray(studentsData) ? studentsData : studentsData?.results || [];

  const createIncidentMutation = useMutation({
    mutationFn: (data: Partial<DisciplineIncident>) => disciplineService.createIncident(data),
    onSuccess: () => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="glass p-8 rounded-[32px] w-full max-w-lg z-10 border border-white/10 shadow-2xl overflow-hidden relative">
        <div className="flex items-center justify-between mb-8">
          <h3 className="text-2xl font-extrabold text-white tracking-tight">Record Incident</h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2">
              <label className="text-[10px] font-black uppercase tracking-widest text-white/40 mb-2 block">Student</label>
              <select 
                value={formData.student} 
                onChange={(e) => setFormData({...formData, student: e.target.value})}
                className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-primary-500/50 transition-all appearance-none"
              >
                {loadingStudents ? (
            ...
            <div>
              <label className="text-[10px] font-black uppercase tracking-widest text-white/40 mb-2 block">Severity</label>
              <select 
                value={formData.category} 
                onChange={(e) => setFormData({...formData, category: e.target.value as DisciplineIncident['category']})}
                className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-primary-500/50 transition-all appearance-none"
              >
          ...
          <div>
            <label className="text-[10px] font-black uppercase tracking-widest text-white/40 mb-2 block">Summary</label>
            <input 
          ...
          <div>
            <label className="text-[10px] font-black uppercase tracking-widest text-white/40 mb-2 block">Detailed Description</label>
            <textarea 
