...
export const StudentProfile = ({ studentId, onClose }: StudentProfileProps) => {
  const { data: student, isLoading: loadingStudent, error: studentError } = useQuery<Student>({
  ...
  const incidents: DisciplineIncident[] = Array.isArray(incidentsData) ? incidentsData : incidentsData?.results || [];
      ...
      <div className="glass h-full w-full max-w-md z-10 border-l border-white/10 shadow-2xl overflow-y-auto animate-in slide-in-from-right duration-300">
        <div className="p-8">
          <div className="flex items-center justify-between mb-8">
            <h3 className="text-2xl font-extrabold text-white tracking-tight">Student Conduct</h3>
          ...
          {loading ? (
            ...
            <div className="space-y-8">
              <div className="flex items-center gap-4 p-4 bg-white/[0.03] rounded-2xl border border-white/5">
                ...
                <div>
                  <h4 className="text-lg font-bold text-white">{student.first_name} {student.last_name}</h4>
                  <p className="text-xs font-black uppercase tracking-widest text-white/30">ID: {student.admission_number}</p>
                  <p className="text-xs text-primary-400/60 mt-1">{student.grade_name} • {student.stream_name}</p>
                </div>
              </div>

              <div>
                <div className="flex items-center justify-between mb-4">
                  <h5 className="text-[10px] font-black uppercase tracking-widest text-white/40">Incident History</h5>
                ...
                {incidents.length > 0 ? (
                  <div className="space-y-3">
                    {incidents.map((it) => (
                      <div key={it.id} className="p-4 bg-white/[0.02] rounded-2xl border border-white/5 hover:border-white/10 transition-all">
                        <div className="flex items-center justify-between mb-2">
                          <span className="text-[10px] font-black text-white/60">{new Date(it.date).toLocaleDateString()}</span>
                          <span className={`text-[10px] font-black uppercase tracking-widest ${it.category === 'SEVERE' ? 'text-red-400' : it.category === 'MAJOR' ? 'text-orange-400' : 'text-primary-400'
                            }`}>
                            {it.category}
                          </span>
                        </div>
                        <h6 className="text-sm font-bold text-white mb-1">{it.summary}</h6>
                        <p className="text-xs text-white/40 leading-relaxed">{it.description}</p>
                        {it.action_taken && (
                          <div className="mt-3 pt-3 border-t border-white/5">
                            <span className="text-[9px] font-black uppercase tracking-widest text-white/20 block mb-1">Action Taken</span>
                            <p className="text-xs text-primary-200/50 italic">"{it.action_taken}"</p>
                          </div>
                        )}
                      </div>
                    ))}
                  ...
                  <div className="text-center py-12 bg-white/[0.01] rounded-2xl border border-dashed border-white/5">
                    <p className="text-xs text-white/20">No disciplinary records found</p>
                  </div>
                )}
              </div>
            </div>
