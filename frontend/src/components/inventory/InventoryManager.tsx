...
export const InventoryManager = () => {
  const [stock] = useState<StockItem[]>([
    ...
    {
      id: "3",
      name: "Textbook: Grade 4 Math",
      category: "LIBRARY",
      quantity: 120,
      unit: "PCS",
      min_threshold: 150,
      last_restock: "2023-12-10",
    },
    {
      id: "4",
      name: "Chalk (White)",
    ...
    <div className="space-y-8 pb-20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {[
          ...
          >
            <div
              className={`w-12 h-12 rounded-2xl ${kpi.bg} flex items-center justify-center ${kpi.color}`}
            >
              <kpi.icon className="w-6 h-6" />
            </div>
            <div>
              <p className="text-[10px] font-black text-primary-200/30 uppercase tracking-widest mb-1">
                {kpi.label}
              </p>
      ...
      <div className="glass p-4 rounded-[28px] border border-white/5 flex flex-wrap gap-4 items-center">
        ...
        <div className="flex gap-2 p-2 bg-white/5 rounded-2xl border border-white/5">
          {["All", "Stationery", "Library", "Lab"].map((tab) => (
            <button
              key={tab}
              className={`px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${tab === "All" ? "bg-primary-500 text-white shadow-premium" : "text-primary-200/40 hover:text-white"}`}
            >
              {tab}
            </button>
          ))}
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr className="bg-white/[0.02] border-b border-white/5">
              <th className="px-8 py-6 text-left text-[10px] font-black text-primary-200/30 uppercase tracking-widest">
                Resource Name
              </th>
              <th className="px-8 py-6 text-left text-[10px] font-black text-primary-200/30 uppercase tracking-widest">
                Category
              </th>
              <th className="px-8 py-6 text-center text-[10px] font-black text-primary-200/30 uppercase tracking-widest">
                Current Stock
              </th>
              <th className="px-8 py-6 text-center text-[10px] font-black text-primary-200/30 uppercase tracking-widest">
                Health
              </th>
              <th className="px-8 py-6 text-right text-[10px] font-black text-primary-200/30 uppercase tracking-widest">
                Last Restock
              </th>
              <th className="px-8 py-6 text-center text-[10px] font-black text-primary-200/30 uppercase tracking-widest">
                Action
          ...
          <tbody>
            {stock.map((item) => (
              ...
              >
                ...
                <td className="px-8 py-6">
                  <span className="text-primary-200/30 font-black text-xs uppercase tracking-widest">
                    {item.category}
                  </span>
                </td>
                ...
                <td className="px-8 py-6 text-center">
                  <div className="flex items-center justify-center gap-2">
                    {item.quantity < item.min_threshold ? (
                      ...
                      <span className="px-3 py-1 bg-emerald-500/10 text-emerald-400 text-[10px] font-black uppercase tracking-widest rounded-lg">
                        Optimal
                      </span>
                    )}
                  </div>
                ...
                <td className="px-8 py-6 text-right">
                  <span className="text-xs text-primary-200/40 font-bold">
                    {item.last_restock}
                  </span>
                </td>
                <td className="px-8 py-6 text-center">
