    ...
    <div className="space-y-8 md:space-y-12 pb-20">
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8">
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
          ...
          <Button onClick={handleSave} disabled={saving || !selectedStream} className="flex-1 lg:flex-none gap-2">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
        <div className="premium-card space-y-4">
          <label className="text-[10px] font-black text-primary-200/30 uppercase tracking-[0.2em]">Operational Date</label>
        ...
        <div className="premium-card space-y-4">
          <label className="text-[10px] font-black text-primary-200/30 uppercase tracking-[0.2em]">Target Deployment</label>
          <select
        ...
        <div className="premium-card space-y-4 sm:col-span-2 lg:col-span-1">
          <label className="text-[10px] font-black text-primary-200/30 uppercase tracking-[0.2em]">Asset Search</label>
          <div className="relative">
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="text-right hidden sm:table-cell">Temporal Stamp</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {loadingStudents || loadingAttendance ? (
              ...
              <TableRow>
                <TableCell colSpan={3} className="text-center py-24 text-primary-200/20 italic text-xs uppercase tracking-widest font-black">
                  {selectedStream ? 'No intelligence records found.' : 'Select a target deployment to initiate tracking.'}
                </TableCell>
              </TableRow>
            ) : (
              filteredStudents.map((s) => (
                <TableRow key={s.id}>
                  ...
                  <TableCell className="text-right text-[10px] font-black text-primary-200/20 uppercase hidden sm:table-cell">
                    {selectedDate === new Date().toISOString().split('T')[0] ? 'Current Cycle' : selectedDate}
                  </TableCell>
                </TableRow>
              ))
            )}
