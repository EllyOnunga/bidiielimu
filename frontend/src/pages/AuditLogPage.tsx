  ...
  <div className="space-y-6 md:space-y-8 pb-20">
    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      <div className="flex items-center gap-3 sm:gap-4">
        ...
        <div>
          <h1 className="text-xl sm:text-2xl md:text-3xl font-black text-primary tracking-tight">Security <span className="text-gradient">Audit Logs</span></h1>
          <p className="text-muted text-xs sm:text-sm">Complete history of system activities and changes.</p>
        </div>
      </div>
        ...
        <table className="w-full text-left min-w-[900px]">
          <thead>
            <tr className="bg-white/5">
              <th className="px-6 py-4 text-xs font-bold text-muted uppercase">Timestamp</th>
              <th className="px-6 py-4 text-xs font-bold text-muted uppercase">User</th>
              <th className="px-6 py-4 text-xs font-bold text-muted uppercase">Action</th>
              <th className="px-6 py-4 text-xs font-bold text-muted uppercase">Module</th>
              <th className="px-6 py-4 text-xs font-bold text-muted uppercase">Target Object</th>
              <th className="px-6 py-4 text-xs font-bold text-muted uppercase text-right">IP Address</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-white/5">
            {loading ? (
              ...
              <tr>
                <td colSpan={6} className="px-6 py-12 text-center text-muted">No logs found.</td>
              </tr>
            ) : (
              logs.map((log, idx) => (
                <motion.tr
                ...
                >
                  ...
                  <td className="px-6 py-4 font-bold text-primary text-sm">{log.user_name}</td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4 text-sm text-muted">{log.object_repr}</td>
                  <td className="px-6 py-4 text-right font-mono text-[10px] text-dim">{log.ip_address}</td>
      ...
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm">
        ...
        >
          <div className="p-6 border-b border-white/5 flex items-center justify-between">
            ...
            <button onClick={() => setSelectedLog(null)} className="p-2 text-muted hover:text-primary transition-all">
          ...
          <div className="p-6 space-y-6 max-h-[70vh] overflow-y-auto custom-scrollbar">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1">
                <p className="text-[10px] font-bold text-muted uppercase tracking-widest">User</p>
                <p className="text-primary font-medium">{selectedLog.user_name}</p>
              </div>
              <div className="space-y-1 sm:text-right">
                <p className="text-[10px] font-bold text-muted uppercase tracking-widest">Timestamp</p>
                <p className="text-primary font-medium">{selectedLog.timestamp}</p>
              </div>
            </div>
            
            <div className="p-4 bg-white/5 rounded-2xl border border-white/5">
              <p className="text-[10px] font-bold text-muted uppercase tracking-widest mb-2">Changes</p>
              {selectedLog.changes ? (
                <pre className="text-xs text-emerald-400 font-mono overflow-x-auto p-2 bg-black/30 rounded-lg">
                  {JSON.stringify(selectedLog.changes, null, 2)}
                </pre>
              ) : (
                <p className="text-sm text-muted italic">No changes recorded for this action.</p>
              )}
            </div>
          </div>
        </motion.div>
