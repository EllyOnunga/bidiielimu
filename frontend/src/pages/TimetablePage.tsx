    ...
    <div className="space-y-6 md:space-y-8 pb-20">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl sm:text-2xl md:text-3xl font-bold text-primary tracking-tight">Smart Timetable</h1>
            <p className="text-muted text-sm md:text-base">Conflict-free scheduling for {selectedClass}</p>
          </div>
        </div>
        ...
        <div className="overflow-x-auto custom-scrollbar">
          <div className="min-w-[1000px]">
            ...
            <div className="grid grid-cols-6 border-b border-white/5 bg-white/5">
              <div className="p-4 border-r border-white/5 text-xs font-bold text-muted uppercase flex items-center justify-center">Time</div>
              {DAYS.map(day => (
                <div key={day} className="p-4 border-r border-white/5 text-xs font-bold text-muted uppercase text-center">{day}</div>
              ))}
            </div>
            ...
            {TIME_SLOTS.map((time) => (
              <div key={time} className="grid grid-cols-6 border-b border-white/5 last:border-0 h-24 md:h-32">
                <div className="p-4 border-r border-white/5 flex flex-col items-center justify-center bg-white/[0.02]">
                  ...
                  <span className="text-[10px] md:text-xs font-bold text-muted">{time}</span>
                </div>
                {DAYS.map(day => {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="glass p-6 rounded-3xl border border-white/5">
          <h2 className="text-lg font-bold text-primary mb-4">Timetable Insights</h2>
          <div className="space-y-4">
            <div className="flex items-center justify-between p-4 rounded-2xl bg-white/5 border border-white/5">
              <div className="flex items-center gap-3">
                ...
                <span className="text-sm text-muted font-medium">Weekly Lesson Count</span>
              </div>
              <span className="text-lg font-bold text-primary">{schedule.length}</span>
            </div>
            <div className="flex items-center justify-between p-4 rounded-2xl bg-white/5 border border-white/5">
              <div className="flex items-center gap-3">
                ...
                <span className="text-sm text-muted font-medium">Completion Rate</span>
              </div>
        ...
        <div className="glass p-6 rounded-3xl border border-white/5">
          <h2 className="text-lg font-bold text-primary mb-4">Conflict Checker</h2>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-sm">
            ...
            >
              <div className="p-6 border-b border-white/5 flex items-center justify-between bg-white/5">
                <h2 className="text-xl font-bold text-primary">Add Schedule Slot</h2>
                <button onClick={() => setIsModalOpen(false)} className="p-2 hover:bg-white/10 rounded-xl text-muted hover:text-primary transition-all">
                  <X className="w-6 h-6" />
                </button>
              </div>
              ...
              <form onSubmit={handleAddSlot} className="p-6 space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-muted">Day</label>
                    <select
                    ...
                    >
                      {DAYS.map((day, i) => <option key={i} value={i} className="bg-bg-color">{day}</option>)}
                    </select>
                  </div>
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-muted">Stream</label>
                    <select
                      value={formData.stream}
                      onChange={(e) => setFormData({ ...formData, stream: e.target.value })}
                      className="w-full px-4 py-2.5 bg-white/5 border border-white/10 rounded-xl text-primary focus:ring-2 focus:ring-primary-500 outline-none"
                    >
                      {streams.map((s: Stream) => <option key={s.id} value={s.id} className="bg-bg-color">{s.grade_name} {s.name}</option>)}
                    </select>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-muted">End Time</label>
                    <input
                      type="time"
                ...
                <div className="space-y-2">
                  ...
                  >
                    ...
                    {subjects.map((s: Subject) => <option key={s.id} value={s.id} className="bg-bg-color">{s.name}</option>)}
                  </select>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-muted">Teacher</label>
                    <select
                      required
                    ...
                    >
                      ...
                      {teachers.map((t: Teacher) => <option key={t.id} value={t.id} className="bg-bg-color">{t.first_name} {t.last_name}</option>)}
                    </select>
                  </div>
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-muted">Room</label>
                    <select
                      value={formData.classroom}
                      onChange={(e) => setFormData({ ...formData, classroom: e.target.value })}
                      className="w-full px-4 py-2.5 bg-white/5 border border-white/10 rounded-xl text-primary focus:ring-2 focus:ring-primary-500 outline-none"
                    >
                      <option value="" className="bg-bg-color">Select Room</option>
                      {classrooms.map((c: Classroom) => <option key={c.id} value={c.id} className="bg-bg-color">{c.name}</option>)}
                    </select>
                  </div>
                </div>
