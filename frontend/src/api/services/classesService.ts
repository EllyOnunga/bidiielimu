...
export const classesService = {
  getGrades: async (search?: string) => {
    const response = await client.get("classes/grades/", {
      params: { search },
    });
  ...
  updateStream: async (
    id: number,
    data: Partial<{ name: string; teacher: number | null }>,
  ) => {
  ...
  getAssignments: async (teacherId?: number) => {
    ...
    const response = await client.get("classes/subject-assignments/", {
      params,
    });
    return response.data;
  },
  ...
  getScheduleSlots: async (streamId?: string | number) => {
    const params = streamId ? { stream: streamId } : {};
    const response = await client.get("classes/schedule-slots/", { params });
    return response.data;
  },
  ...
  getGradeById: async (id: number | string) => {
    const response = await client.get(`classes/grades/${id}/`);
    return response.data;
  },
};
